...
# FILE: stt_test.py
# ─────────────────────────────────────────────────────────────────────────────
# TaleemLine – Speech-to-Text (Offline)
#
# Purpose:
#   Records audio from the microphone and transcribes it to text using
#   OpenAI Whisper running entirely on-device (no API key needed).
#
# Key functions:
#   record_audio(filename, duration, fs)
#     • Records <duration> seconds of mono audio via sounddevice.
#     • Saves to a WAV file using scipy.
#
#   recognize_speech(duration=5)
#     • Calls record_audio(), then loads Whisper "small" model.
#     • Transcribes in Urdu by default (language="ur").
#     • Saves transcript to transcription.txt and returns the string.
#
#   recognize_speech_text_input()
#     • Keyboard fallback — prompts the student to type their question.
#     • Used automatically when microphone recording fails.
#
# Imported by: taleem_main.TaleemAgent.get_student_input
#
# Part of: TaleemLine Voice Learning Agent
# ─────────────────────────────────────────────────────────────────────────────


def record_audio(filename="user_input.wav", duration=10, fs=44100):
