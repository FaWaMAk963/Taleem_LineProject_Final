...
# ─────────────────────────────────────────────────────────────────────────────
# TaleemLine – Student Performance Tracker
#
# Purpose:
#   Records per-session statistics (questions asked, correct/wrong answers,
#   topics covered, duration) and persists them across runs as a JSON file.
#   Provides summary reports for the student at session end.
#
# Key class:
#   PerformanceTracker(student_id)
#     • start_session()      – initialise a new session record
#     • record_question()    – log a question + correct/incorrect outcome
#     • add_topic()          – note a topic covered this session
#     • set_duration()       – record session length in seconds
#     • end_session()        – append to history file, return summary dict
#     • get_progress_report()– formatted multi-line report string
#     • get_overall_stats()  – aggregate stats across all saved sessions
#
# Storage: performance_<student_id>.json (created in the working directory)
#
# Imported by: taleem_main.TaleemAgent (optional integration)
#
